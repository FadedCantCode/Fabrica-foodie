...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Small ball, floats in left area — doesn't block text or form
    mesh.position.set(-1.2, 0.3, -1); mesh.scale.setScalar(0.85); scene.add(mesh);
    const mouse = { x: 0, y: 0 };
    const targetPos = new THREE.Vector3(-1.2, 0.3, -1);
    const currentPos = new THREE.Vector3(-1.2, 0.3, -1);
    ...
    const animate = () => { const e = clock.getElapsedTime(); uniforms.u_time.value = 0.25 * e; uniforms.u_noiseScale.value = Math.sin(e * 0.1) * 0.5 + 1.2; targetPos.set(-1.2 + mouse.x * 0.3, 0.3 + mouse.y * 0.3, -1); currentPos.lerp(targetPos, 0.04); mesh.position.copy(currentPos); renderer.render(scene, camera); rafId = requestAnimationFrame(animate); };
    animate();
